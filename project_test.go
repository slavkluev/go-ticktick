...
func TestGetProject(t *testing.T) {
	client, server := setupTestClient(func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(Project{ID: "proj1", Name: "Project 1"})
	})
	defer server.Close()
...
func TestGetProjectData(t *testing.T) {
	client, server := setupTestClient(func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(ProjectData{
			Project: Project{ID: "proj1", Name: "Project 1"},
			Tasks:   []Task{{ID: "task1", Title: "Task 1"}},
			Columns: []Column{{ID: "col1", Name: "Column 1"}},
		})
	})
	defer server.Close()
...
func TestCreateProject(t *testing.T) {
	client, server := setupTestClient(func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(Project{ID: "proj-new", Name: "New Project"})
	})
	defer server.Close()
...
func TestUpdateProject(t *testing.T) {
	client, server := setupTestClient(func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(Project{ID: "proj1", Name: "Updated"})
	})
	defer server.Close()

	project, err := client.UpdateProject(context.Background(), "proj1", &UpdateProjectRequest{
		Name: String("Updated"),
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
